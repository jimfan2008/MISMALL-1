...
(function(window, $, undefined) {
	...
	})(), replaceInput = ["<div class='sp-replacer'>", "<div class='sp-preview'><div class='sp-preview-inner'></div></div>",
	//"<div class='sp-dd'>&#9660;</div>",
	"</div>"].join(''), markup = (function() {
		...
		return ["<div class='sp-container sp-hidden' style='position: absolute; top: 0px; left: -420px;'>", "<div class='' style='width:100%;height:25px;line-height:25px;font-weight: bold;font-size:16px;border-bottom:1px solid #ccc;'>", "<a title='关闭' class='c_close sp-cancel1' href='javascript:void(0);'></a>", "</div>", "<div class='sp-palette-container'>", "<div class='sp-palette sp-thumb sp-cf'></div>", "</div>", "<div class='sp-picker-container'>", "<div class='sp-top sp-cf'>", "<div class='sp-fill'></div>", "<div class='sp-top-inner'>", "<div class='sp-color'>", "<div class='sp-sat'>", "<div class='sp-val'>", "<div class='sp-dragger'></div>", "</div>", "</div>", "</div>", "<div class='sp-hue'>", "<div class='sp-slider'></div>", gradientFix, "</div>", "</div>", "<div class='sp-alpha'><div class='sp-alpha-inner'><div class='sp-alpha-handle'></div></div></div>", "</div>", "<div class='sp-input-container sp-cf'>", "<input class='sp-input' type='text' spellcheck='false'  />", "</div>", "<div class='sp-initial sp-thumb sp-cf'></div>", "<div class='sp-button-container sp-cf'>", "<a class='sp-cancel' style='' href='#'></a>", "<button class='sp-choose'></button>", "</div>", "</div>", "</div>"].join("");
	})();
	...
	function spectrum(element, o) {
		...
		function initialize() {
			...
			cancelButton1.bind("click.spectrum", function(e) {
				e.stopPropagation();
				e.preventDefault();
			    // hide("cancel");
			    updateOrigin();
		...
		function updateUI() {
		 ...
		 var realColor = get({
				format : format
			}), realHex = realColor.toHexString(), realRgb = realColor.toRgbString();
		...
		function  updateu(){
			...
			if(type=="IMAGEBOX"){
			        restoreColorS = $ctrls.attr("style");
				    restoreColor = $ctrls.find("img").attr("style");
			}else{
				if(type=="BUTTON"){
					 restoreColorB = $ctrls.attr("style");
					 restoreColor = $ctrls.find("input").attr("style");
				}else{
			           restoreColor = $ctrls.attr("style");
			   }
			}
		...
		//点击取消关闭时调用此方法
	     function updateOrigin() {
			...
			if(type=="IMAGEBOX"){
				callbacks.change(restoreColor,restoreColorS);
			}if(type=="BUTTON"){
				callbacks.change(restoreColor,restoreColorB);
			}else{
				callbacks.change(restoreColor);
			}
		}
		function reflow() {
