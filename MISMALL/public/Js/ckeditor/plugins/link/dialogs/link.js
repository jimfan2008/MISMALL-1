...
CKEDITOR.dialog.add("link", function(n) {
	...
	var w = CKEDITOR.plugins.link,
		...
		I = function(a, g) {
			...
			if (!b.type) if (d = c.match(B)) b.type = "anchor", b.anchor = {}, b.anchor.name = b.anchor.id = d[1];
			else if (d = c.match(y)) {
				e = c.match(z);
				c = c.match(A);
				b.type = "email";
	...
	return {
				...
				children: [{
					...
					children: [{
						...
						setup: function(a) {
							...
							(a = this.getDialog().getContentElement("info", "linkType")) && a.getValue() == "anchor" && this.getElement().show();
						},
						commit: function(a) {
		...
		onOk: function() {
			...
			case "url":
				...
				a["pageAddress"] = i.indexOf("/") === 0 ? i : e + i; 
				a["type"] = "url";
				a["data-cke-saved-onClick"] = "return false;";
				break;
			case "anchor":
				//@modify pzh 设置内部页面时添加新的属性
				a["data-cke-saved-href"] ="#"+ (c.anchor && c.anchor.name || e || "");
				a["pageAddress"] = (c.anchor && c.anchor.name || e || "");
				a["type"] = "anchor";
				a["data-cke-saved-onClick"] = "return false;";
				break;
			case "email":
				...
				case "":
				case "encode":
					var i = encodeURIComponent(f.subject || ""),
						h = encodeURIComponent(f.body || ""),
						f = [];
					i && f.push("subject=" + i);
					h && f.push("body=" + h);
					f = f.length ? "?" + f.join("&") : "";
	 ...
	 */
	 function getNavList(){
	   // var navList = "<option value = 0>--请选择--</option>";
		var navList =[["--请选择站内页面--","0"]];
		$(ezGlobal.PageID_S()).find(".ez_page_content").each(function(){
			var pageObj = $(this);
			navList.push([pageObj.attr("page-name"), pageObj.attr("id")]);
		});
		return navList;
	}
});