...
$(function() {
	...
	function forgetPassSendEmail(userName) {
		if (!userName) {
			var userName = $.trim($("#lf_f_uname").val());
		}
		ajax({
			...
			success : function(result) {
				if (result['status'] == 1) {
					...
					var href_email = userName.substring((userName.indexOf("@") + 1), userName.indexOf("."));
					$("#forget-password").find("#forget-password-send").hide();
					$("#forget-password").find("#forget-password-href").show();
					$("#forget-password").find("#forget-password-href a").attr("href", "http://mail." + href_email + ".com").text(userName);
					$("#forget-password #forget-password-href button:first").click(function() {
						window.open("http://mail." + href_email + ".com", "_self");
					});
		...
		ajax({
			...
			success : function(r) {
				...
				if (r['status'] == 1 || r['status'] == 2) {
					if (userName == "15280@qq.com") {
						$("#btn_dev").remove();
						$("#panel_logined").find('a').after('<div id="btn_dev">设计者</div>');
					}
					$("#panel_logined").find("a:first").text(r["message"]);
					//登录成功之后的特效
					var left = parseFloat($("#panel_login").offset().left) + 60;
					var top = parseFloat($("#panel_login").offset().top) + 30;
					$("#loginmodal .layer").hide();
					$("#loginmodal").animate({
						height : "0",
						width : "0",
						left : left + "px",
	...
	$("#registerform").submit(function() {
		var userName = $.trim($("#lf_r_uname").val());
	...
	//忘记密码点击发送按钮发送邮件
	$("#forget-password").find("#forget-password-send button:first").click(function() {
		if (userFlag) {
			forgetPassSendEmail();
		}
	});
