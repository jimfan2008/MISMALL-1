...
Util.Drag.prototype = {
	...
	init : function(container,drag,options){
		...
		this.Move = ImagePlay.bindAsEventListener(this,this.move);
		this.Stop =ImagePlay.bindAsEventListener(this,this.stop);
		this.drag.style.cursor = "move";	
		ImagePlay.addEvent(this.drag,ImagePlay.bindAsEventListener(this,this.start),"mousedown");
	},
	start : function(E){	
		this.x = E.clientX - this.drag.offsetLeft;
		this.y = E.clientY - this.drag.offsetTop;		
		ImagePlay.addEvent(document,this.Move,"mousemove");
		ImagePlay.addEvent(document,this.Stop,"mouseup");
		this.startFunc.call(this,E, this.container);
		if(document.all){
			ImagePlay.addEvent(this.drag, this.Stop, "losecapture");
			this.drag.setCapture();
			E.cancelBubble = true;
		}else{
			ImagePlay.addEvent(window, this.Stop, "blur");
			E.preventDefault();
		}
	},
	move : function(E){
	...
	stop : function(E){
		ImagePlay.removeEvent(document,this.Move,"mousemove");
		ImagePlay.removeEvent(document,this.Stop,"mouseup");
		this.endFunc.call(this,E, this.container);
		if(document.all){
			ImagePlay.removeEvent(this.drag, this.Stop, "losecapture");
			this.drag.releaseCapture();
			E.cancelBubble = true;
		}else{
			ImagePlay.removeEvent(window, this.Stop, "blur");
		}
	}
};
...
Slider.prototype = {
	...
	init : function(options){
		...
		this.icontainer = ImagePlay.get(options.icontainer);
		this.idrag =ImagePlay.get(options.idrag);
		if(options.plusBtn){
			this.plusBtn =ImagePlay.get(options.plusBtn);
			this.reduceBtn = ImagePlay.get(options.reduceBtn);			
		}	
		this.direction = options.direction;
		if(options.content){
			this.content = ImagePlay.get(options.content);
			this.panel =ImagePlay.get(options.panel);
	...
	addEvent : function(){
		...
		self.fixedBug();
		if(self.plusBtn){
			ImagePlay.addEvent(self.plusBtn, function(){
				self.goToward("up", self);
			}, "mousedown");
			ImagePlay.addEvent(self.plusBtn, function(){
				clearTimeout(self.timer);
			}, "mouseup");	
			ImagePlay.addEvent(self.reduceBtn, function(){
		...
		ImagePlay.addEvent(self.icontainer, function(event){
		...
		new Util.Drag(self.icontainer, self.idrag, {
			...
			moveFunc : function(E, idrag, icontainer){
				if(self.content){
					self.callback(parseInt(ImagePlay.getStyle(idrag, self.direction)), self.Max[self.direction]);
				}
			},
			endFunc : function(E){		
				setTimeout(function(){
	...
	run : function (self){
		...
		var posUp = Math.min(Math.max(parseInt(ImagePlay.getStyle(self.idrag, self.direction)) + acc,self.Min[self.direction]),self.Max[self.direction]);
		if(self.content){
	...
	mouseClick : function(event, self){
		var pos = ImagePlay.getPos(event),
			offset = ImagePlay.getS(self.idrag),
			dis = pos[self.direction == "top" ? "pageY" : "pageX"] - offset[self.direction],
			top = parseInt(ImagePlay.getStyle(self.idrag, self.direction)),
		...
		if(self.content){
			self.callback(parseInt(ImagePlay.getStyle(self.idrag, self.direction)), self.Max[self.direction]);	
		}
		ImagePlay.preventDefault(event);	
	},	
	scrollFunc : function scrollFunc(event, self){
		self.dec =ImagePlay.getDetail(event);
		self.isScroll = true;
		self.run();
		ImagePlay.preventDefault(event);
	},	
	keydown : function scrollFunc1(event, self){
		var which = ImagePlay.getWhich(event);
		if(which === 38){
			self.dec = - 10;
		}
	...
	fixedBug : function(){
		...
		if(!document.all){
			ImagePlay.addEvent(this.icontainer, function(){
