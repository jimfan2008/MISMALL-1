	...
	var AjaxgetAppstore = {
		editorByTplId : function(tempType, siteId, name, imgSrc) {
			...
			if (AjaxgetAppstore.isLogin()) {
				...
				//跳转登录
					window.location =URLPATH +"/login.html";
			}
		},
		isLogin : function() {
			var status = false;
		...
		getAppstoreHtml : function(data) {
			...
			if (data["saleApps"]) {
				...
				$.each(data["saleApps"], function(key, value) {
					...
					apptoreHtml.push('<div class="col-md-3 col-sm-6 templated" ><div class="temp-border" style=""><div class="img" ><img style="width:100%;height:183px" alt="" src="');
					...
					apptoreHtml.push('</strong>的作品</span></div><div class="row optBtnWrap"><span class="template-details col-md-6"><a class="btn btn-info active no-radius" href="../Unregistered/shop_detail.html?id='+value["id"]+'" data-filter="*">详情</a></span>');
					if(!isMob){
