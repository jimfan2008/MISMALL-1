...
define(function(require, exports, module) {
	var htmlStr = '<div class="row"><div class="col-md-6"><span class="selectInnerLink selectLink link" style="display:block;width:100%;text-align:center;font-size:16px;line-height:35px;background:#00ccff;color:#ffffff;cursor: pointer;">内部链接</span></div><div class="col-md-6"><span class="selectOuterLink link" style="display:block;width:100%;text-align:center;font-size:16px;line-height:35px;background:#f5f5f5;color:#ccc;cursor: pointer;">外部链接</span></div></div><div class="outerLink" style="display:none;margin-top:10px"><div class="row"><div class="col-md-3" style="padding-top:7px">链接地址  </div><div class="col-md-9"><input class="outLinkValue form-control" placeholder="请填写链接地址"></div></div></div><div class="innerLink"><div class="row"  style="margin-top:15px;"><div class="col-md-3" style="padding-top:7px">选择页面</div><div class="col-md-9"><select class="innerPage form-control"></select><div class="action-caret page-caret"></div></div></div><div class="col-md-12" style="margin-top:15px;padding:0px;border-top:1px solid #ccc"><button class="btn btn-default input-sm addCondition" style="margin-top:5px;margin-bottom:5px" flag="false">添加条件</button><button class="btn btn-default btn input-sm addLinkParam" type="button" style="margin:10px;line-height:2px;">添加参数</button></div><div class="row condition"></div><div class="linkParam" style="margin-top:10px;padding-top:10px;border-top:1px solid #ccc"></div></div>';
	var pageJump = {
		action : function(actionIdx) {
			var actionId = actionIdx ? actionIdx : "pageJump" + ezCommon.actionNameNum[ezCommon.formId];
			...
			$actionHtml.find(".actionTitle").text("页面跳转");
			var optionHtml = "";
		...
		json : function($actionObj) {
			...
			if ($(".selectLink", $actionObj).hasClass("selectOuterLink")) {
				...
				if (pageId) {
					ctrlJson = '"pageId":"' + pageId + '","linkType" : "innerLink"';
				}
			...
			var logicStruck ="";
			if(pageJump.logicJson){
				logicStruck = ',"logic" : '+pageJump.logicJson+',"logicStr" : "'+pageJump.logicStr+'"';
			}
			console.log(logicStruck);
			var ctrlActionJson = '"' + actionId + '":{"type":"pageJump","ctrl":{' + ctrlJson + '},"paramItem":[' + paramJson + '],"whereJump":' + whereJump +logicStruck+'}';
			return ctrlActionJson;
		},
		//还原
		restore : function(actionJson, actionName) {
			var $actionHtml = pageJump.action(actionName), pageJumpType = "", ctrl = actionJson["ctrl"];
			ezCommon.actionClose($actionHtml);
			pageJumpType = ctrl["linkType"];
			if (pageJumpType == "outerLink") {
			...
			pageJump.actionName = actionJson["logicStr"];
		...
		logic : {
			...
			leftStruck : function($obj) {
				return '<div class="handleLogic-content" funcName="pageJump" type="inner"><span>选择页面</span> <span style="border: 1px solid #ccc;display: inline-block;height: 25px;margin-left: 10px;padding: 3px 0 0;text-align: center;width: 119px;">-- 请选择 --</span></div><div class="handleLogic-content" funcName="pageJump" type="outer"><span>链接地址</span><span style="border: 1px solid #ccc;display: inline-block;height: 25px;margin-left: 10px;padding: 3px 0 0;text-align: center;width: 119px;">填写链接地址</span></div>';
			},
			...
			save : function(logicJson,$action){
				...
				pageJump.logicJson = JSON.stringify(logicJson);
				pageJump.logicStr=logicStr.replace(/"/g,"'");
				console.log(pageJump.logicStr);
