...
var system = {
	...
	_addSysFields : function() {
		...
		$(".right_where_column").find("input.normal").on("keydown", function(e) {
			...
			if ((kc == 8 || kc == 46) && $(this).attr("immark") != "true") {
				//当是从插件点击插入的数据按回退(8)和delete(46)时候直接清空  immark==ture为键盘输入否则为插件输入的
				$(this).val("");
		...
		$(".right_where_column").find("input.normal").on("click", function(e) {
			...
			obj.focus();
			//$(this).parents("li:first").addClass("li-condition");
			$(this).attr("data-toggle","dropdown");
			obj.after($dropDown);
			$('#menu-right').on('hidden.bs.dropdown', function () {
			 		//$(this),remove();
			});
	...
	_initGetSql : function() {
		...
		//var editTname = system.initGetTname();
		//判断是多表还是单表
		if ($("#left .menuSelected").attr("id") == "byForm") {//此时是单表
			...
			var strWhere = "(1 = 1 ";
				...
				//循环获取条件
				$(".right_where_column").find("li.dropdown:visible").each(function(index) {
					
					//判断是否隐藏了
					//if ($(this).css("display") != "none") {

						//需要判断是否是带时间插件的
						if($(this).find("input").val() != ''){//如果为空就算了
							if ( $(this).attr("fname").substring(0, 6) != "CCTime" && $(this).find("input").val() != '' && $(this).attr("add") != "add") {//不为空 第一条 不是时间
								...
								var link_str = system.getConditionValue($(this).find("select").val());
									...
									switch($(this).find("input").attr("datafrom")) {
										case "input":
											trueValue =" '" + $(this).find("input").val() + "' ";
											
											break;
										case "sysParam":
											trueValue = " #" + $(this).find("input").attr("field") + "# ";
											break;
										case "form":
											trueValue = "  $" + $(this).find("input").attr("field") + "$";
											break;
									...
									if(link_str=="like")
									trueValue = "%"+trueValue+"%";
									strWhere += ") and (" + system.gettrueCon(strTname, $(this).attr("fname"), $(this).attr("ftype")) + " " + link_str + trueValue;
								...
								var link_str = system.getConditionValue($(this).find("select:last").val());
									...
									switch($(this).find("input").attr("datafrom")) {
										case "input":
											linkTrueValue = "'" + $(this).find("input").val() + "'";
											break;
										case "sysParam":
											linkTrueValue =  "#" + $(this).find("input").attr("field") + "#";
											break;
										case "form":
											linkTrueValue = " $" + $(this).find("input").attr("field") + "$ ";
											break;
									...
									if(link_str == "like")
									linkTrueValue = "%"+linkTrueValue+"%";
									strWhere += " " + strlianjie + " " + system.gettrueCon(strTname, $(this).attr("fname"), $(this).attr("ftype")) + link_str + linkTrueValue;
								
	
							} else if ($(this).attr("fname").substring(0, 6) == "CCTime" &&  $(this).attr("add") != "add") {
								alert(2);//时间 非增加  不为空 
								...
								if ($(this).attr("set") == "value") {
									...
									var sval=$(this).find("select").val()*1;
									if(sval<7){
										strWhere += ") and (" + system.gettrueCon1(strTname, $(this).attr("fname"), $(this).attr("ftype")) + arr[sval]+" '" + $val + "' ";
									}else{
										strWhere += ") and (" + system.gettrueCon(strTname, $(this).attr("fname"), $(this).attr("ftype")) + " like '%" + $val + "%' ";
									}
							...
							} else if ($(this).attr("fname").substring(0, 6) == "CCTime" && ($(this).find(".form-control").val() ) && $(this).attr("add") == "add") {//时间 非空 增加
								if ($(this).prevAll("li[add=no]").eq(0).find("input.form-control ").val() == "" ) {
