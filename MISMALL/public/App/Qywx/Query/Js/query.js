...
var QueryClass = function() {
    ...
    this._init = function() {
        ...
        //flowid=" + _this.flowId +"&formid=" + _this.oFormId + "&type='interface'",
        if (type == "") {
        ...
        if ($getXml != "") {
            var frmID = $getXml.children("control[type='CCForm']").attr("name");
        }
                ...
                success : function(data) {
                    ...
                    for (var key in obj) {
                        if (obj[key]["display"] != "none") {
                            html_content = $('<li fieldname="' + obj[key]["value"] + '" type="' + obj[key]["type"] + '"><span class="title">' + obj[key]["name"] + '' + '</span>' + '<span class="delete"></span><div class="opt"><a><img src="'+PUBLICPATH+'/App/Designer/Query/Images/px.png" title="按此列排序" class="orderBythis"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/fz.png" title="按此列分组" class="cat" />'
                        +'<img src="'+PUBLICPATH+'/App/Designer/Query/Images/qh.png" title="按此列求和" class="sum"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/max.png" title="求最大值" class="max"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/min.png" title="求最小值" class="min"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/pj.png" title="求平均值" class="avg"/></a></div>'
                            +' </li>');
                    ...
                    xml_query.find("fields field").each(function() {
                        ...
                        if (xml_cond.size() == 0) {//判断条件是否为空
                            ...
                            if ($(this).find("value").html().substr(0, 6) == "CCTime") {//判断是否是时间字段
                                html_conditions += '<li add="no" ftype="' + $(this).find("type").html() + '" fname="' + $(this).find("value").html() + '" id="CTime"><span class="tit">' + $(this).find("name").html() + '</span>';
                                html_conditions += strConditionLast1;
                                $(".right_where_column").append(html_conditions);
                                $.parser.parse($("#CTime"));
                                $("#CTime").removeAttr("id");
                            } else {
                                html_conditions += '<li add="no" ftype="' + $(this).find("type").html() + '" fname="' + $(this).find("value").html() + '"><span class="tit">' + $(this).find("name").html() + '</span>';
                            ...
                            if ($thisValue.substr(0, 6) == "CCTime") {//字段为时间的时候
                                xml_cond.each(function(index) {//条件为空的时候
                                    ...
                                    if (index == 0) {//第一个条件的显示字段的额名字
                                        var html_conditions = "";
                                        html_conditions += '<li add="no" ftype="' + $thisType + '" fname="' + $thisValue + '" id="CTime"><span class="tit">' + $thisShowName + '</span>';
                                        html_conditions += strConditionLast1;
                                        $(".right_where_column").append(html_conditions);
                                        $.parser.parse($("#CTime"));
                                        ...
                                        var html_conditions = "";
                                        html_conditions += '<li add="add" ftype="' + $thisType + '" fname="' + $thisValue + '" id="CTime"><span class="tit" style="background:#FFF;">' + $thisShowName + '</span>';
                                ...
                                xml_cond.each(function(index) {//条件不为空字段不是时间的时候
                                    if (index == 0) {//第一个条件的显示字段的名字
                                        var html_conditions = "";
                                        html_conditions += '<li add="no" ftype="' + $thisType + '" fname="' + $thisValue + '"><span class="tit">' + $thisShowName + '</span>';
                                        html_conditions += strConditionLast;
                                        $(".right_where_column").append(html_conditions);
                                        ...
                                        var html_conditions = "";
                                        html_conditions += '<li add="add" ftype="' + $thisType + '" fname="' + $thisValue + '" ><span class="tit"  style="background:#fff;">' + $thisShowName + '</span>';
                                        html_conditions += '<select style="width:68pt;"><option value="or">或者</option><option value="and">并且</option></select>';
        ...
        function _initTitle() {
            ...
            _commonAjax({
                ...
                url : GROUPPATH + "/Query/getFlowDataTableTitle",
                dataType : 'text',
                success : function(r) {
                    var json_titleobj = jQuery.parseJSON(r);
                    var strTitle = '';
                    ...
                    $(json_titleobj).each(function(index) {
                        strConditionTitle += '<li add="no" ftype="' + json_titleobj[index].controlType + '" fname="' + json_titleobj[index].fieldName + '" style="display:none"><span class="tit">' + json_titleobj[index].fieldTitle + '</span>';
                        if (json_titleobj[index].fieldName.substr(0, 6) == "CCTime") {
                            strConditionTitle += strConditionLast1;
                        } else {
                            strConditionTitle += strConditionLast;
                        }
                        strTitle += '<li fieldName=' + json_titleobj[index].fieldName + ' type="' + json_titleobj[index].controlType + '"><span class="title">' + json_titleobj[index].fieldTitle + '</span><span class="delete"></span><div class="opt"><a><img src="'+PUBLICPATH+'/App/Designer/Query/Images/px.png" title="按此列排序" class="orderBythis"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/fz.png" title="按此列分组" class="cat" />'
                        +'<img src="'+PUBLICPATH+'/App/Designer/Query/Images/qh.png" title="按此列求和" class="sum"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/max.png" title="求最大值" class="max"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/min.png" title="求最小值" class="min"/><img src="'+PUBLICPATH+'/App/Designer/Query/Images/pj.png" title="求平均值" class="avg"/></a></div></li>';
                    });
                    $(".right_data_header").empty();
                    $('.right_data_header').append(strTitle);
                    $(".right_where_column").empty();
        ...
        function _initAddSql() {
            ...
            $(".right_where_column").find(".addCondition").on("click", function() {
                ...
                $(this).parent().parent().find("li[fname='" + fname + "']").each(function() {
                    //如果不是时间为空
                    ...
                    } else if (fname.substr(0, 6) == "CCTime" ) {//如果是时间为空
                      
                       // alert($(this).find("input.easyui-datetimebox").datetimebox("getValue"));
                       // if($(this).find("input.easyui-datetimebox").datetimebox("getText") != "" || $(this).find("input.easyui-datetimebox").datetimebox("getValue") !=""){
                        if ($(this).find("input.easyui-datetimebox").datetimebox("getValue") == "" && $(this).find("input.easyui-datetimebox").datetimebox("getText") == ""  ){
                            mark = false;
                            var obj = $(this).find(".addCuo");
                            obj.fadeIn();
                            setTimeout(function() {
                ...
                if (mark == true) {
                    ...
                    if (fname.substr(0, 6) == "CCTime") {
                        $(this).parent().parent().find("li[fname='" + fname + "']").last().after('<li fname="' + fname + '" ftype="' + ftype + '" add="add" id="addConditions">' + '<span style="background:#fff;" class="tit">&nbsp;</span>' + '<select style="width:68pt;"><option value="or">或者</option>' + '<option value="and">并且</option></select>' + '<select><option value="1">等于</option>' + '<option value="2">大于等于</option>' + '<option value="3">小于等于</option>' + '<option value="4">大于</option>' + '<option value="5">小于</option>' + '<option value="6">不等于</option>' + '<option value="7">包含</option>' + '</select><input  data-options="showSeconds:true,editable:false" class="easyui-datetimebox" ' + 'style="width:210px;height:30px;line-height:30px;font-size:15px;margin-left:0px;" />' + '<img class="delCondition" ' + 'style="cursor:pointer;margin-left:10px;margin-top:8px;" src="' + IMAGES_URL + 'icon_sjydel.png" />' + '<a class="addCuo">请先填写筛选条件</a></li>');
                        $.parser.parse($("#addConditions"));
                        $("#addConditions").removeAttr("id");
                    } else {
                        $(this).parent().parent().find("li[fname='" + fname + "']").last().after('<li fname="' + fname + '" ftype="' + ftype + '" add="add" >' + '<span style="background:#fff;" class="tit">&nbsp;</span>' + '<select style="width:68pt;"><option value="or">或者</option>' + '<option value="and">并且</option></select>' + '<select><option value="1">等于</option>' + '<option value="2">大于等于</option>' + '<option value="3">小于等于</option>' + '<option value="4">大于</option>' + '<option value="5">小于</option>' + '<option value="6">不等于</option>' + '<option value="7">包含</option>' + '</select><input type="text" class="normal" ' + 'oninput="system.changeDataFromType($(this))" datafrom="" field="" immark="" value="" /><img class="delCondition" ' + 'style="cursor:pointer;margin-left:10px;margin-top:8px;" src="' + IMAGES_URL + 'icon_sjydel.png" />' + '<a class="addCuo">请先填写筛选条件</a></li>');
                    }
        ...
        function addSysFieldsControl($tname, $tid, $getTname) {
            ...
            _commonAjax({
                ...
                success : function(result) {
                    ...
                    $(paras_obj).each(function(index) {
                        $("#mm").menu('appendItem', {
                            parent : document.getElementById('menu-sysParam'),
                            id : paras_obj[index]['variableName'],
                            text : paras_obj[index]['variableDesc'],
                        });
                    });
            ...
            //解析xml start
            var frm = $getXml.children("control[type=CCForm]").attr("name");
            $getXml.children("control[type='CCTabs']").each(function() {
                //获取选项卡控件列表节点ID
                var elementId = $(this).children("property[name='CCTabElements']:first").attr("value");
                //alert(elementId);
                //遍历选项卡控件
                //根据选项卡控件列表，遍历对应的选项卡控件
                $getXml.children("#" + elementId).children().each(function() {
                    ...
                    if (pdObj != "CCTable" && pdObj != "CCUploa" && pdObj != "CCSearc" && pdObj != "CCButto" && pdObj != "CCLabel" && pdObj != "CCImage") {
                        var $controlName = $getXml.children("control[name='" + $controlValue + "']:first").children("property[name=Title]:first").attr("value");
                        //添加到sys列表中
                        $("#mm").menu('appendItem', {
                            parent : document.getElementById('menu-forminfo'),
                            text : $controlName,
                        ...
                        var $controlTableObj = $getXml.children("control[name='" + $controlValue + "']:first");
                        //拿到table的ID
                        ...
                        //table的控件遍历
                        $getXml.children("#" + tableElements).children().each(function() {
                            ...
                            if (streId != "CCLabel" && streId != "CCButto" && streId != "CCSearc" && streId != "CCUploa" && streId != "CCTable" && streId != "CCImage") {
                                var contTableVal = $getXml.children("control[name=" + eId + "]").children("property[name=Title]").attr("value");
