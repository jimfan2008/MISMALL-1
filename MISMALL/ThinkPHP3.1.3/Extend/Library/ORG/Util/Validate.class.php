...
class Validate {
    ...
    /**
     * 验证数据 支持 in between equal length regex expire ip_allow ip_deny idcard
	...
	'username'	=>	'/^[a-zA-Z][a-zA-Z0-9_]{4,19}$/i', //用户名称专用,字母开头,使用数字字母下划线，共5-20位
        	'password'	=>	'/^\S{5,20}$/i',	//用户密码专用,匹配任何非空白字符,共5-20位
        );
        // 检查是否有内置的正则表达式
     ...
     * @access public
     * @param string $value 15位或18位身份证号码
     * @param mixed $len 长度，可选，默认为both，如果不为both,那么15位号码无效
     * @return mixed 如果失败，返回false,成功，则返回一个18位的身份证号码
     **/
    public static function idcard($value, $len = 'both')
    {
        if((15 === strlen($value)) && ('both' === $len)) {
                $trueNum = substr($value,0,6).'19'.substr($value,6);
                //为返回18位号码作准备。
                $preg = "/^[\d]{8}((0[1-9])|(1[0-2]))((0[1-9])|([12][\d])|(3[01]))[\d]{3}$/";
        } elseif(18 === strlen($value)) {
                $trueNum = substr($value,0,17);
