...
class Think {
    ...
    static private function buildApp() {
        ...
        if(C('APP_TAGS_ON')) {
            ...
            }else{ //[cluster] 默认加载系统行为扩展定义
                C('extends', include CLUSTER_PATH.'Conf/tags.php');
            }
        }
        ...
        if(isset($mode['core'])) {
			...
			$list  =  array(
				//[cluster] 核心文件列表
                CLUSTER_PATH.'Common/functions.php', // 标准模式函数库
                CORE_PATH.'Core/Dispatcher.class.php', // URL调度类
                CORE_PATH.'Core/App.class.php',   // 应用程序类
                CLUSTER_PATH.'Lib/Core/Action.class.php', // 控制器类
