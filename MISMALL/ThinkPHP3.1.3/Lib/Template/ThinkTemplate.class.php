...
class  ThinkTemplate {
    ...
    private function replaceBlock($content){
        ...
        $reg2  = '/('.$begin.'block\sname=[\'"](.+?)[\'"]\s*?'.$end.')(.*?)'.$begin.'\/block'.$end.'/eis';
        if(is_string($content)){
            do{
                ...
                //TODO php5.2
                do{
                    preg_match($reg2, $content, $matches);
                    $content2 = $this->replaceBlock($matches);
                    $content = str_replace($matches[0], $content2, $content);
                }while (preg_match($reg2, $content, $matches));
            ...
            if(preg_match('/'.$begin.'block\sname=[\'"](.+?)[\'"]\s*?'.$end.'/is', $content[3])){ //存在嵌套，进一步解析
                ...
                //TODO php5.2
                do{
                    preg_match($reg2, "{$content[3]}{$begin}/block{$end}", $matches);
                    $content2 = $this->replaceBlock($matches);
                    $content[3] = str_replace($matches[0], $content2, "{$content[3]}{$begin}/block{$end}");
                }while (preg_match($reg2, "{$content[3]}{$begin}/block{$end}", $matches));
