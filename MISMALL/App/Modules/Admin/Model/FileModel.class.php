...
class FileModel extends Model {
	...
	protected $_auto = array(
		...
		array('location', 0, self::MODEL_BOTH),
	);
	 ...
	 */
	public function upload($setting) {
		...
		if ($info) {
			...
			foreach ($info as $key => &$value) {
				...
				$value['savepath'] = substr($setting['rootPath'], 1); //在模板里的url路径
				/* 记录文件信息 */
				if ($this->create($value) && ($id = $this->add())) {
					$value['id'] = $id;
					$value['path'] = $value['savepath'] . $value['savename'];
					$value['url'] = C('UPLOAD_URL') . $value['path'];
				} else {
