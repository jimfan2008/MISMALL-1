...
class AdminListBuilder extends AdminBuilder {
    ...
    public function display() {
        ...
        $this->convertKey('uid', 'text', function($value){
            $value = query_user(array('nickname','uid'), $value);
            return "[{$value[uid]}]".$value['nickname'];
        });
        ...
        $pager = new Page($this->_pagination['totalCount'], $this->_pagination['listRows'], $_REQUEST);
        $pager->setConfig('theme','%first% %upPage% %linkPage% %downPage% %end% %header%');
        $pager->setConfig('header', '<span class="rows">共 %totalRow% 条记录</span>');
