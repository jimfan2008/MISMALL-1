...
class CategoryAction extends AdminAction {
    ...
    public function index(){
        $tree = D('Category')->getTree(0,'id,name,title,sort,pid,status');
    ...
    public function remove(){
        ...
        //判断该分类下有没有内容
        $document_list = M('article')->where(array('category_id'=>$cate_id))->field('id')->select();
        if(!empty($document_list)){
            $this->error('请先删除该分类下的文章（包含回收站）');
        }

        //删除该分类信息
        $res = D('Category')->deleteCategory($cate_id);
        if($res !== false){
            //记录行为
