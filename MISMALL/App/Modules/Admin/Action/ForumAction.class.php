...
class ForumAction extends AdminAction {
    ...
    public function editForum($id=null) {
        ...
        $builder
            ...
            ->keyId()->keyTitle()->keyCreateTime()
            ->keyMultiUserGroup('allow_user_group', '允许发帖的用户组')
            ->keyStatus()
            //->keyImage('logo','板块图标','用于显示的图标')
            ->keyHidden('logo')
            ->data($forum)
    ...
    public function doEditForum($id=null, $title, $create_time, $status, $allow_user_group,$logo) {
        ...
        if(empty($title)){
        	$this->error('板块名称不能为空');exit;
        }
        if(empty($allow_user_group)){
        	$this->error('请选择用户组');
        }
        ...
        //生成数据
        $data = array('title'=>$title, 'create_time'=>strtotime($create_time), 'status'=>$status, 'allow_user_group'=>$allow_user_group,'logo'=>$logo);
        ...
        //返回成功信息
        $this->success($isEdit ? '编辑成功' : '保存成功', U('Forum/forum'));
    }
