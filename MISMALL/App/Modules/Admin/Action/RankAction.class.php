...
class RankAction extends AdminAction
{
    ...
    public function index($page = 1, $r = 20)
    {
        ...
        foreach ($list as &$val) {
            $val['u_name'] = M('FroumMember')->where('uid=' . $val['uid'])->getField('nickname');
        }
    ...
    public function deleteRank($id = null)
    {
        ...
        $result = M('FroumRank')->where('id=' . $id)->delete();
        $result1 = M('FroumRankUser')->where('rank_id=' . $id)->delete();
        if ($result) {
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
    ...
    public function editRank($id = null)
    {
        ...
        //如果是编辑模式
        if ($isEdit) {
            $rank = M('FroumRank')->where(array('id' => $id))->find();
        }
        //显示页面
        $builder = new AdminConfigBuilder();
        $builder
            ->title($isEdit ? '编辑头衔' : '新增头衔')
            ->keyId()->keyTitle()->keyImage('logo', '头衔图标', '图标')
            ->data($rank)
    ...
    public function userList()
    {
        ...
        $list = $this->lists('FroumMember', $map);
        int_to_string($list);
    ...
    public function userRankList($id = null, $page = 1)
    {
        ...
        $u_name = M('FroumMember')->where('uid=' . $id)->getField('nickname');
        $model = M('FroumRankUser');
        ...
        foreach ($rankList as &$val) {
            $val['title'] = M('FroumRank')->where('id=' . $val['rank_id'])->getField('title');
    ...
    public function userChangeRank($id = null)
    {
        ...
        $data = M('FroumRankUser')->where('id=' . $id)->find();
        if (!$data) {
            $this->error('该头衔关联不存在');
        }
        $ranks = M('FroumRank')->select();
    ...
    public function doUserAddRank($id = null, $uid, $reason, $is_show, $rank_id)
    {
        ...
        if ($is_Edit) {
            ...
            if (!$result) {
                ...
                $rank = M('FroumRank')->where('id=' . $data['rank_id'])->find();
                //$logoUrl=getRootUrl().D('picture')->where('id='.$rank['logo'])->getField('path');
                $u_name = M('ForumMember')->where('uid=' . $uid)->getField('nickname');
                $content = '管理员给你颁发了头衔：[' . $rank['title'] . ']'; //<img src="'.$logoUrl.'" title="'.$rank['title'].'" alt="'.$rank['title'].'">';

                $user = query_user(array('nickname', 'space_link'), $uid);

                $content1 = '管理员给@' . $user['nickname'] . ' 颁发了新的头衔：[' . $rank['title'] . ']，颁发理由：' . $reason; //<img src="'.$logoUrl.'" title="'.$rank['title'].'" alt="'.$rank['title'].'">';
    ...
    public function deleteUserRank($id = null)
    {
        ...
        $result = M('FroumRankUser')->where('id=' . $id)->delete();
        if ($result) {
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
