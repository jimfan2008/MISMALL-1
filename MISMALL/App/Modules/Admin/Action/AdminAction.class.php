...
class AdminAction extends BaseAction
{
	...
	protected function _initialize()
	{
        ...
        if ( $access === false ) {
            ...
            if( $dynamic === null ){
                //检测非动态权限
                $rule  = strtolower(GROUP_NAME.'/'.MODULE_NAME.'/'.ACTION_NAME);
    ...
    final protected function accessControl(){
		...
		$allow = C('ALLOW_VISIT');
		$deny  = C('DENY_VISIT');
		$check = strtolower(MODULE_NAME.'/'.ACTION_NAME);
     ...
     * 设置一条或者多条数据的状态
     */
    public function setStatus($Model=MODULE_NAME){
     ...
     */
    final public function getMenus($controller = MODULE_NAME)
    {
        ...
        if(empty($menus)){
            ...
            if($current){
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if( stripos($item['url'],GROUP_NAME)!==0 ){
                        $item['url'] = GROUP_NAME.'/'.$item['url'];
                    }
                    // 判断主菜单权限
    ...
    final protected function returnNodes($tree = true){
        ...
        if((int)$tree){
            ...
            foreach ($list as $key => $value) {
                if( stripos($value['url'],GROUP_NAME)!==0 ){
                    $list[$key]['url'] = GROUP_NAME.'/'.$value['url'];
                }
            }
            ...
            foreach ($nodes as $key => $value) {
                if( stripos($value['url'],GROUP_NAME)!==0 ){
                    $nodes[$key]['url'] = GROUP_NAME.'/'.$value['url'];
                }
            }
        }
