...
//判断$text是否是图片路径
	...
	static function thumb($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight = 'auto', $interlace = true) {
		...
		if ($scale >= 1) {
			...
			// 缩略图尺寸
			$cutWidth = (int) ($srcWidth * $scale);
			$cutHeight = (int) ($srcHeight * $scale);
		}

		return Image::crop($image, $thumbname, $cutWidth, $cutHeight, $type, 0, 0, $srcWidth, $srcHeight, $interlace);
	}
	 ...
	 * @return void
	 */
	static function thumb2($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight = 'auto', $interlace = true) {
		// 获取原图信息
		$info = Image::getImageInfo($image);
		if ($info === false) {
			return false;
		...
		if ($maxHeight == 'auto') {
			$scale = $maxWidth / $srcWidth;
			$cutHeight = $srcHeight * $scale;
		}

		return Image::crop($image, $thumbname, $cutWidth, $cutHeight, $type, $srcX, $srcY, $srcWidth, $srcHeight, $interlace);
	}
	 ...
	 * @param string $image  原图
	 * @param string $thumbname 缩略图文件名
	 * @param string $cutWidth 宽度
	 * @param string $cutHeight 高度
	 * @param string $type 图像格式
	 * @param int $srcX 裁剪X坐标
	 * @param int $srcY 裁剪Y坐标
	 * @param string $srcWidth 原图宽度
	 * @param string $srcHeight 原图高度
	 * @param boolean $interlace 启用隔行扫描
	 * @return void
	 */
	static function crop($image, $thumbname, $cutWidth, $cutHeight, $type = '', $srcX = 0, $srcY = 0, $srcWidth = '', $srcHeight = '', $interlace = true) {
		// 获取原图信息
		$info = Image::getImageInfo($image);
		if ($info === false) {
			return false;
		}
		$srcWidth = $srcWidth ? $srcWidth : $info['width'];
		$srcHeight = $srcHeight ? $srcHeight : $info['height'];
