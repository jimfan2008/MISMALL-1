<?php
/*
 * function : 用户信息类，对应用户数据库基础信息，注册一个用户
 * time : 201306280910
 * */
class InitUser {
	...
	public function getUserInfo($condition) {
		$userObj = D("users");
		$user_info = $userObj -> where($condition) -> find();
		unset($userObj);
	...
	public function addUser($userinfo) {
		...
		$data['userPwd'] = $userinfo['userpwd'];
		$data['userEmail'] = $userinfo['email'];
		$data['IsActivated'] = 0;

		$userObj = D("users");
		$uid = $userObj -> add($data);
		unset($data, $userObj);
	...
	public function addLoginInfo($user) {
		...
		$data['loginIP'] = gethostbyname($_ENV['COMPUTERNAME']);
		//当前登录用户IP
	...
	public function userLogin($user_name, $user_pwd) {
		$userInfo = $this -> getUserInfo("userName='$user_name' and userPwd='" . md5($user_pwd) . "'");

		if ($userInfo) {

			import('ORG.Users.Company');
			 ...
			 */
			$sess_data = array('uid' => $userInfo['ID'], 'uname' => $userInfo['userName'], 'uemail' => $userInfo['userEmail'], 'company' => $companyInfo, );
	...
	public function setSession($session_data) {
		...
		//		//session过期时间
		//		$session_expire = time()+C('SESSION_EXPIRE');
		//
		//		$sessObj = D('session');
		//		$data = array(
		...
		//		if($sess_id){
		//			$sessInfo = $sessObj->where(array('session_id' => $sess_id))->find();
		//			if($sessInfo && $sessInfo['session_expire']){
		//				$sessObj->where(array('session_id' => $sess_id))->save($data);
		//			}else{
		//				$data['session_id']  = $sess_id;
		//				$sessObj->add($data);
		//			}
		//		}else{
		//			$data['session_id'] = $sess_id = session_id();
		//			$sessObj->add($data);
		//		}

		//设置统一登录cookie
		//		import('ORG.Users.Company');
		//		setcookie(C('SESSION_COOKIE_NAME'), $sess_id, $session_expire, "/", Company::getCookieDomain());
		//		//TODO server http_host获取不到域名，暂时设置双cookie
		//		if(HTTP_HOST == '10.28.5.37'){
		//			setcookie(C('SESSION_COOKIE_NAME'), $sess_id, $session_expire, "/", '.wclouds.net');
		//		}
	}
	...
	public function getSessionInfo() {
		/*
		 ...
		 //session过期
		 if( $sessInfo['session_expire'] > time()+C('SESSION_EXPIRE') ){
		 return false;
		 }
		 //session数据
		 $userInfo = unserialize($sessInfo['session_data']);

		 //TODO考滤到程序中很多地方用到原生session
		...
		if (isset($_SESSION['uid']) && $_SESSION['uid'] != '') {
			$userInfo = array('uid' => $_SESSION['uid'], 'uname' => isset($_SESSION['uname']) && $_SESSION['uname'] ? $_SESSION['uname'] : null, 'company' => isset($_SESSION['company']) && $_SESSION['company'] ? $_SESSION['company'] : null, );
			return $userInfo;
		} else {
			return false;
		}
	...
	public function logout() {
		...
		//TODO server http_host获取不到域名，暂时设置双cookie
		if (HTTP_HOST == '10.28.5.37') {
			setcookie(C('SESSION_COOKIE_NAME'), '', time() - 1, "/", '.wclouds.net');
		}
	}
