...
final class UserBase {
	...
	public function getUserInfo($post) {
		//查询条件
		$where = array(
			'ID' => isset($post['user_id']) && $post['user_id'] ? intval($post['user_id']) : '',
			'userName' => isset($post['username']) && $post['username'] ? trim($post['username']) : '',
			'userEmail' => isset($post['email']) && $post['email'] ? trim($post['email']) : '',
		);
		//过滤空项
	...
	public function login($username, $password) {
		...
		$info = $this->_user_model->field('ID, userName, userPwd, isActivated, lastStatus')->where($where)->find();
	...
	public function edit($user_id, $post) {
		...
		isset($post['shareCode']) && $post['shareCode'] ? $set['shareCode'] = trim($post['shareCode']): '';
		isset($post['lastStatus']) && $post['lastStatus'] ? $set['lastStatus'] = trim($post['lastStatus']): '';
		isset($post['userPhone']) && $post['userPhone'] ? $set['userPhone'] = trim($post['userPhone']): '';
		isset($post['province']) && $post['province'] ? $set['province'] = trim($post['province']): '';
		isset($post['city']) && $post['city'] ? $set['city'] = trim($post['city']): '';
		isset($post['district']) && $post['district'] ? $set['district'] = trim($post['district']): '';
		isset($post['address']) && $post['address'] ? $set['address'] = trim($post['address']): '';
		isset($post['idCardFront']) && $post['idCardFront'] ? $set['idCardFront'] = trim($post['idCardFront']): '';
		isset($post['idCardBack']) && $post['idCardBack'] ? $set['idCardBack'] = trim($post['idCardBack']): '';
		isset($post['idCardHand']) && $post['idCardHand'] ? $set['idCardHand'] = trim($post['idCardHand']): '';
		isset($post['userQQ']) && $post['userQQ'] ? $set['userQQ'] = trim($post['userQQ']): '';
		...
		public function updateUserInfo($uid,$userInfo){
			$userInfo['isValidated']=1;
			$re = $this->_user_model->where(array('ID' => $uid))->save($userInfo);
			//print_r($this->_user_model->getLastSql());
			return $re;
		}
