...
class Upload {
	...
	/**
	 * 本地图片上传到云服务器不能通过is_uploaded_file() 函数
	 * @author cjli
	 * @time(2015-05-30)
	 * @var boolean
	 */
	private $is_check_move_file = true;

	/**
	 * 构造方法，用于构造上传实例
	 * @param array  $config 配置
	 * @param string $driver 要使用的上传驱动 LOCAL-本地上传驱动，FTP-FTP上传驱动
	 * @param boolean $is_check_move_file 禁止is_uploaded_file() 函数检测
	 */
	public function __construct($config = array(), $driver = '', $driverConfig = null, $is_check_move_file = '') {
		/* 获取配置 */
		...
		$this->is_check_move_file = is_bool($is_check_move_file) ? $is_check_move_file : true;
		/* 设置上传驱动 */
		// $class      =   strpos($driver,'\\')? $driver : 'Think\\Upload\\Driver\\'.ucfirst(strtolower($driver));
		$class = ucfirst(strtolower($driver));
		include 'Upload/Driver/' . $class . '.class.php';//$class=Oss
		// print_r($driverConfig);
		// print_r($this->$config);
		// print_r("-----");
	...
	public function uploadOne($file) {
		$info = $this->upload($file ? array($file) : '');
	...
	public function upload($files = '') {
		...
		foreach ($files as $key => $file) {
			...
			/* 检测并创建子目录 */
			$subpath = $this->getSubPath($file['savename']);
			if (false === $subpath) {
				continue;
			} else {
