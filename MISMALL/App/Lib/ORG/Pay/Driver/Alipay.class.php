...
class Alipay extends PayBase {
    ...
    protected $config = array(
        'type' => 1,  //到账类型 1:支付宝担保交易, 2:即时到账交易
        'email' => '',
        'key' => '',
        'partner' => ''
    );
        ...
        if($this->config['type'] == 2) {
            ...
            //支付宝担保交易
                $param = array(
                    ...
                    "quantity"  => 1,
                    "logistics_fee" => isset($otherParam['logistics_fee']) ? $otherParam['logistics_fee'] : '0.00',                     //物流费用
                    "logistics_type"    => isset($otherParam['logistics_type']) ? $otherParam['logistics_type'] : 'EXPRESS',               //物流类型
                    "logistics_payment" => isset($otherParam['logistics_payment']) ? $otherParam['logistics_payment'] : "SELLER_PAY",    //物流支付方式
                );
                // 收货人姓名
                if(isset($otherParam['receive_name']) && $otherParam['receive_name']) {
                    $param['receive_name'] = $otherParam['receive_name'];
                }
                // 收货人地址
                 if(isset($otherParam['receive_address']) && $otherParam['receive_address']) {
                    $param['receive_address'] = $otherParam['receive_address'];
                }
                // 收货人邮编
                 if(isset($otherParam['receive_zip']) && $otherParam['receive_zip']) {
                    $param['receive_zip'] = $otherParam['receive_zip'];
                }
                // 收货人电话号码
                 if(isset($otherParam['receive_phone']) && $otherParam['receive_phone']) {
                    $param['receive_phone'] = $otherParam['receive_phone'];
                }
                // 收货人手机号码
                 if(isset($otherParam['receive_mobile']) && $otherParam['receive_mobile']) {
                    $param['receive_mobile'] = $otherParam['receive_mobile'];
                }
        }

        //商品展示地址
        if(isset($otherParam['show_url']) && $otherParam['show_url']) {
                    $param['show_url'] = $otherParam['show_url'];
        }
        ksort($param);
        reset($param);
