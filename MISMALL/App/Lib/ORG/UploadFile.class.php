...
//类定义开始
		...
		if ($this->thumb && in_array(strtolower($file['extension']), array('gif', 'jpg', 'jpeg', 'bmp', 'png'))) {
			...
			if (false !== $image) {
				...
				for ($i = 0, $len = count($thumbWidth); $i < $len; $i++) {
					...
					if (false !== $ret) {
						file_upload($tmp_file, $thumbPath . $thumbname . '.' . $thumbExt);
					}
	...
	public function upload($savePath = '') {
		...
		if ($isUpload) {
			...
			return $this->uploadFileInfo = $fileInfo;;
		} else {
			$this->error = '没有选择上传文件';
			return false;
		}
...
//没有定义命名规则，则保持文件名不变
			...
			if (function_exists($rule)) {
				$saveName = getTimeStr().random(5).".".$filename['extension'];
				//使用函数生成一个唯一文件标识号
...
/*
		...
		/**/
		if ($this->saveSubPath) {
			$tmpPath = C('UPLOAD_PATH').$this->saveSubPath;
			...
			$saveName = $this->saveSubPath . '/' . $saveName;
		}
		return $saveName;
	}
